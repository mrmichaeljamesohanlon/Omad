...
Start a local web server in this folder with `python3 -m http.server 8000`, then open <http://localhost:8000>. The page loads the recipe list from `recipes-v2.json`; opening `index.html` directly may block that fetch.
...
`recipes-v2.json` is the file the website reads. `recipes.json` is kept as a compatibility copy. `generate_catalog.py` rebuilds both copies of the 205-entry catalogue and its rough nutrition estimates from inexpensive meal patterns; rerunning it replaces manual edits to the generated JSON.
